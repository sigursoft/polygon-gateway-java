...
public class ExchangeRateService {
	...
	public Mono<ExchangeRate> provideExchangeRate(String buyCurrency, String sellCurrency) {
		Mono<PolygonForexResponse> polygonRate = polygonWebClient.fetchPreviousDayExchangeRate(buyCurrency,
				sellCurrency);
		return polygonRate.flatMap(polygonForexResponse -> Mono.just(polygonForexResponse.results().get(0).vw()))
